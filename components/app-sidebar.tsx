  ...
  return (
    <Sidebar className="border-r border-gray-200" {...props}>
      ...
      <SidebarContent className="gap-1 px-2">
        <SidebarMenu className="flex space-x-4 py-4">
          {navItems.map((item) => {
            ...
            return (
              <SidebarMenuItem key={item.path} className="py-1">
                {/* Completely skip SidebarMenuButton and use your own button/link */}
                <Link
                  href={item.path}
                  className={cn(
                    "flex items-center gap-3 px-4 py-3 rounded-md transition-colors duration-300 w-full",
                    isActive
                      ? "bg-secondary text-white lg:border-l-4 border-[#FFC98E] rounded-lg"
                      : " text-[#6C757D] hover:text-secondary"
                  )}
                >
                  <span className="text-inherit hover:scale-125 transition-transform duration-300">
                    {item.icon}
                  </span>
                  <span className="text-inherit text-sm">{item.name}</span>
                </Link>
              </SidebarMenuItem>
            );
