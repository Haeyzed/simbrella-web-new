...
import serviceImageBG from "@/public/images/serviceBox.png";
...
export function ServiceSectionTab({
  services,
  canEdit,
  onUpdate,
}: ServiceSectionTabProps) {
  const [selectedService, setSelectedService] = useState<ServiceSection | null>(
    null
  );
  ...
  const handleDelete = async (serviceId: number) => {
    try {
      ...
      toast("Error", { description: "Failed to delete service" });
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {canEdit && (
          <DrawerDialog
            trigger={
              <button className="p-2 justify-center items-center text-sm text-secondary bg-white border-orange-200 border-2 rounded-lg hover:bg-secondary hover:text-white cursor-pointer">
                Add New Service
              </button>
            }
            ...
            onOpenChange={(open) =>
              !selectedService && setIsEditDialogOpen(open)
            }
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {services.length === 0 ? (
          ...
          services.map((service, index) => (
            <div
              key={service.id}
              className="relative p-4 bg-white rounded-xl border shadow-sm flex flex-col justify-between"
              style={{
                backgroundImage: `url(${serviceImageBG.src})`,
            ...
            >
              <div className="flex items-start gap-3 mb-4">
                <div className="w-10 h-10 flex-shrink-0">
                  <Image
                    src={
                      index % 3 === 0 ? icon1 : index % 3 === 1 ? icon2 : icon3
                    }
                    alt="icon"
                    width={40}
                    height={40}
                ...
                <div>
                  ...
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: stripHtmlTags(service.summary),
                    }}
                  />
                </div>
              </div>
              ...
              {canEdit && (
                <div className="flex gap-2 mx-auto">
                  ...
                  <button
                    className="flex text-xs items-center space-x-3 px-3 py-1 bg-white border-[1px] border-orange-200 hover:bg-red-700 text-gray-800 hover:text-white rounded-lg cursor-pointer"
                    onClick={() => {
                      setSelectedService(service);
      ...
      {selectedService && (
        <DrawerAlertDialog
          ...
          description={`Are you sure you want to delete "${selectedService.title}"?`}
...
//Hayzed's implementation of the ServiceSectionTab component
