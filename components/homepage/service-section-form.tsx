...
export function ServiceSectionForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(
        `Error ${service?.id ? "updating" : "creating"} service:`,
        error
      );
      toast("Error", {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="mb-6 w-3/5">
        ...
        >
          ...
          <input
            ...
            type="file"
            accept="image/*"
            className="hidden w-full border-gray-300 rounded-md"
            onChange={handleIconChange}
          />
        </div>
        <p className="text-xs text-gray-500 mt-1">
          Recommended size: 64x64px, not more than 500kb
        </p>
      </div>
      ...
      <div className="flex items-center gap-4">
        <button className="cursor-pointer border-2 border-gray-200 hover:bg-red-600 hover:text-white rounded-lg px-3 py-2 text-sm" onClick={onSuccess}>
          Cancel
        </button>
        <button
          type="submit"
