...
export function HeroSectionForm({
  ...
  const handleSubmit = async (e: React.FormEvent, isDraft = false) => {
    ...
    try {
      ...
      formData.append(
        "status",
        isDraft ? "draft" : isActive ? "published" : "draft"
      );
      ...
      if (response?.success) {
        toast("Success", {
          description: `Hero section ${
            heroSection?.id ? "updated" : "created"
          } successfully`,
        });
        onSuccess?.();
      } else {
        toast("Error", {
          description:
            response?.message ||
            `Failed to ${heroSection?.id ? "update" : "create"} hero section`,
        });
      }
    } catch (error) {
      console.error(
        `Error ${heroSection?.id ? "updating" : "creating"} hero section:`,
        error
      );
      toast("Error", {
        description: `Failed to ${
          heroSection?.id ? "update" : "create"
        } hero section`,
      });
    } finally {
    ...
    <div className="space-y-6 mt-10 lg:w-[955px] ">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-semibold">
          {heroSection?.id ? "Edit Hero Section" : "Create Hero Section"}
        </h1>
      </div>
        ...
        <form onSubmit={(e) => handleSubmit(e, false)}>
          ...
          <div className="mb-8 w-3/5">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Note : minimum of 3 images not more than 500kb
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <Button
              type="button"
              variant="secondary"
              className="bg-gray-500 hover:bg-gray-400 text-white cursor-pointer"
